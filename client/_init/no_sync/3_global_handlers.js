...
//isRTL used for right-to-left orientation in the UI
Handlebars.registerHelper('isRTL', function() {
    return site_prefs.isRTL;
});
...
// Provides price formating according to configuration
// TODO Need to incorporate automatic price conversion from configuration
Handlebars.registerHelper('formatPrice', function(block) {
    //console.log(this)

    if (site_prefs.currencySymbolPosition == "left") {
        return Template.formatPriceLeft(block);
        //in global_handlers.html
    } else {
        return Template.formatPriceRight(block);
        //in global_handlers.html
    }
});
...
// Provides insertion of currency symbol per configuration
function insertCurrencySymbol(total_price) {
    if (site_prefs.currencySymbolPosition == "left") {
        return Template.formatPriceLeft(total_price);
        //in global_handlers.html
    } else {
        return Template.formatPriceRight(total_price);
        //in global_handlers.html
    }
}
