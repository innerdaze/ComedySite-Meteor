...
(($) ->
  ...
  $.fn.pathchange = (handler) ->
    if handler then this.bind "pathchange", handler else this.trigger "pathchange"
  ...
  my = $.pathchange =
    ...
    options:
      ...
      pollingInterval : 25    # when using Hashchange in browsers without it, how often to poll the hash (in ms)
    ...
    init: (options) ->
      ...
      if my.options.useHistory and my.detectHistorySupport()
      then $(window).bind "popstate", (e) ->
        if my.historyReady
      ...
      if my.options.useHashchange
        ...
        if not my.detectHashchangeSupport()
    ...
    detectHistorySupport: () ->
      not not (window.history && history.pushState)

    # Simple feature detection for hashchange (adapted from Modernizr)
    detectHashchangeSupport: () ->
      isSupported = "onhashchange" in window
      if not isSupported and window.setAttribute
        window.setAttribute "onhashchange", "return;"
        isSupported = typeof window.onhashchange is "function";
      isSupported
